...
[발행]
  ...
  /raw_image : sensor_msgs/Image             D435i RGB 원본 영상 (대시보드용)
"""
...
class VisionNode(Node):
    def __init__(self):
        ...
        self._raw_image_pub = self.create_publisher(Image,             '/raw_image', 10)
    ...
    # raw_image 발행
    # ----------------------------------------------------------
    def _publish_raw_image(self, img):
        try:
            ros_img = self.bridge.cv2_to_imgmsg(img, encoding='bgr8')
            ros_img.header.stamp    = self.get_clock().now().to_msg()
            ros_img.header.frame_id = 'camera_color_optical_frame'
            self._raw_image_pub.publish(ros_img)
        except Exception as e:
            self.get_logger().warn(f'/raw_image 발행 실패: {e}')
