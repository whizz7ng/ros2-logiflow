...
GRIP_OFFSET = {
    1: [-20.4, -2.0, 78.0],    # 1층 z만 조정 (위에서 잡히니 낮춰야)
    2: [-53.4, -2.0, 51.0],      #  2층 (현재 값, 정확)
}
...
Y_COMP_1F_NEG = 15.0   # 1층 y<0(오른쪽)일 때 더할 양
        ...
        try:
            ...
            if self.current_level == 1:
                ...
                if not self._check_gripped():
                    ...
                    # brain_node가 이걸 받고 /observe_move부터 다시 시작
                ...
                # ===== 2층: 정면 앞 → 전진 → 파지 =====
                APPROACH_X_2F = 50.0   # 블록 앞 40mm에서 시작 (실측 조정)
